...
func TestDyingMemcgReclaimWithAbsolutePath(t *testing.T) {
	...
	t.Run("reclaim with threshold", func(t *testing.T) {
		...
		mockey.Mock(GetManager).Return(fakeManager).Build()
		mockey.Mock((*FakeCgroupManager).GetCgroupNrDyingDescendants).To(func(_ *FakeCgroupManager, _ string) (int, error) {
			callCount++
			if callCount == 1 {
		...
		mockey.Mock(MemoryOffloadingWithAbsolutePath).To(func(_ context.Context, _ string, _ int64, _ machine.CPUSet) error {
			offloadCount++
			return nil
		}).Build()
		err = DyingMemcgReclaimWithAbsolutePath(context.TODO(), tmpDir, metrics.DummyMetrics{}, "entry", "container", machine.NewCPUSet(0))
